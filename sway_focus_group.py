...
st.set_page_config(page_title="SWAY Focus Group", page_icon="🎭", layout="wide")

# ============================================
# PASSWORD PROTECTION
# ============================================
SWAY_PASSWORD = "SwayRocks"
...
def check_password():
    ...
    <div style="text-align: center; padding: 3rem;">
        <h1>🎭 SWAY Focus Group Simulator</h1>
        <h3>Practice Your Pitch with AI Personas</h3>
        <p style="color: #666;">Built for SWAY Academy</p>
    </div>
...
You're dealing with high turnover, inconsistent quality across locations, and managers who don't communicate. You WANT help but have zero bandwidth.""",
        "opener": "*checking phone* Hey, sorry - walk with me to my next meeting. You've got maybe five minutes. What's SWAY and why should I care?"
    },
    "detail_david": {
...
You run a clinic struggling with interdepartmental communication, long patient wait times, and physician burnout. You're open to change but need to understand exactly how it works.""",
        "opener": "I've read a bit about Agile methodologies but I'm curious how SWAY differs, particularly for healthcare settings. Can you walk me through the framework's core components and how they'd apply to a clinical environment?"
    },
    "friendly_fiona": {
        "name": "Friendly Fiona",
...
You have a chaotic creative team, missed deadlines, and scope creep on every project. You KNOW you need help but avoid confronting it directly.""",
        "opener": "Oh hi! I've heard such great things about SWAY from a friend! I'm so excited to chat. Tell me everything - I'm super curious!"
    }
}
...
def build_persona_system_prompt(persona, knowledge_text=""):
    ...
    if knowledge_text:
        base += f"\n\nCONTEXT: The consultant may reference the following information about SWAY:\n{knowledge_text[:10000]}"
...
def get_feedback(conversation, persona, api_key):
    ...
    feedback_prompt = f"""You are a sales coach evaluating a SWAY consultant's practice pitch session. 
...
with st.sidebar:
    ...
    if uploaded_doc:
        try:
            knowledge_text = uploaded_doc.getvalue().decode('utf-8')
    ...
    <div style="text-align: center; color: #666; font-size: 0.8rem;">
        <em>SWAY Focus Group Simulator</em><br>
        <em>Built for SWAY Academy</em>
    </div>
    """, unsafe_allow_html=True)
...
# ============================================
    ...
    with col3:
        ...
        export_text = f"""# SWAY FOCUS GROUP — SESSION REPORT
## {timestamp}
## Built for SWAY Academy
...
"""
        ...
        st.download_button(
            "📥 Export Report",
            export_text,
            file_name=f"SWAY_SESSION_{date_stamp}.md",
            mime="text/markdown",
